...
export const connectState = (stateHandler, dispatchHandler) => {
  return connect(state => typeof stateHandler === "function" ? stateHandler(state[constants.APP_NAMESPACE]) : {}, dispatchHandler);
}; // Create redux actions that namespaces the widget editor

export const createAction = action => VzCreateAction(`${constants.APP_NAMESPACE}/${action}`); // Gets an action with our namespace. used for our sagas
