...
function* storeAdapterConfigInState({
  payload
}) {
...
function* resolveWithProxy() {
  const {
    widgetEditor
  } = yield select(); // Check and patch current state based on user configuration

  const proxyResult = yield call([stateProxy, "sync"], widgetEditor); // --- Proxy results returns a list of events we call on certain updates
...
function* updateWidgetData() {
  ...
  if (widgetData.data) {
    ...
    if (!widgetEditor.editor.initialized) {
      yield put({
        type: constants.sagaEvents.DATA_FLOW_VISUALISATION_READY
      });
    }
  }
}
...
export default function* baseSaga() {
  ...
  yield takeLatest(constants.sagaEvents.DATA_FLOW_FIELDS_AND_LAYERS_READY, updateWidgetData); // --- Triggered once: When we have all nessesary information to render visualization

  yield takeLatest(constants.sagaEvents.DATA_FLOW_VISUALISATION_READY, preloadData); // --- Triggered once: When App initializes, we store adapter configuration localy

  yield takeLatest(constants.sagaEvents.DATA_FLOW_STORE_ADAPTER_CONFIG, storeAdapterConfigInState); // --- Triggered multiple: When Configuration or data updates we update the widget

  yield takeLatest(constants.sagaEvents.DATA_FLOW_UPDATE_WIDGET, updateWidget); // --- Triggered multiple: When configuration updates, we update widget data

  yield takeLatest(constants.sagaEvents.DATA_FLOW_CONFIGURATION_UPDATE, updateWidgetData); // --- Triggered multiple: When configuration gets modified, we check with our state
  // --- proxy if we have updates, if thats the case we update the editors state based on response

  yield takeLatest(getAction("CONFIGURATION/patchConfiguration"), resolveWithProxy); // --- Triggered multiple: If theme gets modified we update our widget
